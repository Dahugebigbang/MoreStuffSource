...
public class ItemSpeedC extends Item  {
	
	public ItemSpeedC(int i){
	
	
	super();                        
	this.setHasSubtypes(true);                        
	this.setMaxDamage(8);
	this.setUnlocalizedName("SpeedC");
	this.setCreativeTab(MoreStuff.TabMoreStuff);
	
	this.setTextureName(Strings.MODID + ":" + "SpeedC");
	}
	...
	public ItemStack onItemRightClick(ItemStack item, World w, EntityPlayer e){
		...
		if(!w.isRemote)	
		{
			...
			item.damageItem(1, e);
			e.addPotionEffect(new PotionEffect(Potion.moveSpeed.id, 5 * 100, 1, true));
		}
