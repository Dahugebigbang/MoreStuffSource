...
public class ItemWBC extends Item {
	
	
	
public ItemWBC(int i){
		
		
		super();                        
		this.setHasSubtypes(true);                        
		this.setMaxDamage(8);
		this.setUnlocalizedName("WBC");
		this.setCreativeTab(MoreStuff.TabMoreStuff);
		
		
		this.setTextureName(Strings.MODID + ":" + "WBC");
		}
		...
		public ItemStack onItemRightClick(ItemStack item, World w, EntityPlayer e){
			...
			if(!w.isRemote)	
			{
				...
				item.damageItem(1, e);
				e.addPotionEffect(new PotionEffect(Potion.waterBreathing.id, 5 * 100, 1, true));
			}
